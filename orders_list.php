...
/**
 * 2019 GrupoImpultec Gestión Logística
 ...
 * NOTICE OF LICENSE
 *
 *  @author    Carlos Tornadijo GrupoImpultec S.L.
 *  @copyright 2019 GrupoImpultec.
 *  @license   Commercial
 *  @version 1.0.1
 */
...
function grupoimpultec_my_render_list_page() {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'crear_envio') {
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'preparar_crear_envio') {
            ...
            $porcentaje_reembolso = sanitize_text_field($_GET['pr']);
...
if (!function_exists('grupoimpultec_preparar_crear_envio')) {
    ...
    function grupoimpultec_preparar_crear_envio($numero_pedido_wp, $id_agencia, $importe, $porcentaje_reembolso, $porcentaje_seguro, $numero_bultos_defecto, $array_bultos_defecto, $direccion_remitente, $permite_reembolsos, $permite_recoger, $permite_no_recoger, $maxima_cantidad_reembolso, $minima_cantidad_reembolso, $maxima_cantidad_seguro, $tipo_cliente, $iva_exento, $iva, $servicio_recogida, $id_agencia_madre, $agencia_mapa_origen, $agencia_mapa_destino) {
        ...
        if (strlen($datos_array['iso_pais_salida']) != 2) {
            switch ($GLOBALS['id_pais_api']) {
                case 1:
                    ...
                    break;
                case 2:
                    $datos_array['iso_pais_salida'] = 'PT';
                    break;
                case 12:
                    $datos_array['iso_pais_salida'] = 'IT';
                    break;
                case 4:
                    $datos_array['iso_pais_salida'] = 'DE';
                    break;
                case 7:
        ...
        if ($agencia_mapa_destino == 1 || $id_agencia_madre == 1) {
            ...
            $oficina_destino_seleccionada_por_cliente = grupoimpultec_pedido_tiene_oficina_destino($numero_pedido_wp);
            ...
            if (grupoimpultec_chrono_tiene_entrega_destino($numero_pedido_wp)) {
                echo('<script>');
...
function grupoimpultec_pedido_tiene_oficina_destino($numero_pedido_wp) {
    $pedido = wc_get_order($numero_pedido_wp);
...
function grupoimpultec_chrono_tiene_entrega_destino($numero_pedido_wp) {
    $pedido = wc_get_order($numero_pedido_wp);
